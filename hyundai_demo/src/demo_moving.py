...
def create_root():
    ...
    #
    # Scene #2  (Go to the table and head tilt down.)
    #

    scene2 = py_trees.composites.Sequence("move_to_table")

    wait_scene2_intro = py_trees_ros.subscribers.CheckData(name="wait_intro_demo", topic_name="/wait_select_scene", topic_type=String,
           variable_name="data", expected_value="scene2")

    start_scene2 = PrintState_Behaviour(name="* Scene2 *")
    scene2_say1 = PrintState_Behaviour(name="Moving to table.")
    ...
    # goal_table.target_pose.pose.position.x = 3
    # goal_table.target_pose.pose.position.y = 1.4
    ...
    # goal_table.target_pose.pose.orientation.z = 0.707
    # goal_table.target_pose.pose.orientation.w = 0.707
    goal_table.target_pose.pose.orientation.x = 0
    goal_table.target_pose.pose.orientation.y = 0
    goal_table.target_pose.pose.orientation.z = 0.999
    goal_table.target_pose.pose.orientation.w = 0.044

    move_to_table = py_trees_ros.actions.ActionClient(
        name="move to table",
        action_namespace="/move_base",
        action_spec=move_base_msgs.msg.MoveBaseAction,
        action_goal=goal_table
    )
    head_tilt_down = MoveJoint(name="tilt_down", controller_name="/tilt_controller", command=0.6)
    ...
    wait_go_home_intro = py_trees_ros.subscribers.CheckData(name="wait_intro_demo", topic_name="/wait_select_scene", topic_type=String,
           variable_name="data", expected_value="go_home")

    start_go_home = PrintState_Behaviour(name="* go_home *")
    go_home_say1 = PrintState_Behaviour(name="Moving to home position.")
    ...
    goal_home.target_pose.pose.position.x = 1
    ...
    goal_home.target_pose.pose.orientation.z = 0.707
    goal_home.target_pose.pose.orientation.w = 0.707
...
#          move_manipulator_to_grasp3,
    ...
    scene7 = py_trees.composites.Sequence("find_object")
    wait_scene7_intro = py_trees_ros.subscribers.CheckData(name="wait_intro_demo", topic_name="/wait_select_scene", topic_type=String,
           variable_name="data", expected_value="scene7")
    start_scene7 = PrintState_Behaviour(name="* Scene7 *")
    find_object = QRCodeActionClient(
        name="find_qr_code",
        action_namespace="/qrcode_detect",
        action_spec=QRCodeDetectAction,
    ...
    root.add_children([init_joint, gripper_close_cmd, gripper_open_cmd, fold_arm, scene1, scene2, scene3, scene4, scene5, scene6, scene7, go_home])
