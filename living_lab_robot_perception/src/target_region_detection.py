...
class SegModule:    # U2Net + HSNet
    ...
    def RGBD_sub_callback(self, image_msg, depth_msg):
        ...
        if self.target_id > 0:
            image = np.frombuffer(image_msg.data, dtype=np.uint8).reshape(image_msg.height, image_msg.width, -1).copy()
            depth = np.frombuffer(depth_msg.data, dtype=np.float32).reshape(depth_msg.height, depth_msg.width, -1).copy()
            ...
            # cv2.imshow("Result", salient_detection_img)
            # cv2.imshow("Input", segmented_image)
            ...
            query_img = PIL_Image.fromarray(ori_img.copy())
            ...
            other_region = salient_detection_img.astype(np.uint8)
